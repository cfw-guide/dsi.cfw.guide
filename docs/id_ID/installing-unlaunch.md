...
Mohon [buat dump NAND](dumping-nand.html) dulu **jika belum**. A NAND backup + [ntrboot](https://wiki.ds-homebrew.com/ds-index/ntrboot) (or a [hardmod](https://wiki.ds-homebrew.com/ds-index/hardmod), provided you know how to solder) would allow you to restore this backup in case the console gets bricked afterwards.
...
Jika tidak punya komputer, atau punya komputer ChromeOS, mohon jangan pasang Unlaunch. A PC (running Windows, Linux, or macOS) is required to ensure the SD Card used is formatted in a way compatible with Unlaunch.

:::

::: warning

Unlaunch tidak kompatibel dengan konsol pengembang Nintendo DSi.
...
7. If Unlaunch is already installed, select the `Uninstall unlaunch` or `Restore launcher tmd` option, and press <kbd class="face">A</kbd> once it's done
   - If you only want to **uninstall** Unlaunch, you can stop here
8. Select the `Install unlaunch` option, and press the <kbd class="face">A</kbd> button
9. Press the <kbd class="face">A</kbd> button once installation is done
10. Press <kbd class="face">POWER</kbd> to reboot your system
...
1. Nyalakan daya konsol selagi menahan <kbd class="face">A</kbd> dan <kbd class="face">B</kbd>
...
2. Navigate to `OPTIONS`, and look at the available options
   - <kbd class="face">A</kbd> + <kbd class="face">B</kbd> sudah terkode tetap di menu Unlaunch, jadi tidak bisa diubah
