...
1. In the DSi Menu, launch the System Settings application (the icon is gray and has a white wrench)
    ...
    - If the version is *not* `Ver 1.4.2`, go straight to [Section III](#section-iii-installing-unlaunch)
...
1. If you want to disable the DSi menu music, select `Disable DSi menu sound` and press <kbd class="face">A</kbd> to turn it off
1. If you want to disable the DSi splash (with health and safety message), select `Disable DSi menu H&S` and press <kbd class="face">A</kbd> to turn it off
...
1. Navigate to `OPTIONS`, and select the `NO BUTTON` option
1. In the list that follows, select `Launcher`
