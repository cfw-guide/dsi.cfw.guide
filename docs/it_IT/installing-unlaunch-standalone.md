# Installare Unlaunch Standalone
...
4. Per disabilitare la musica del menu DSi, seleziona `Disable DSi menu sound` e premi <kbd class="face">A</kbd> per confermare
5. Per disabilitare l'immagine d'avvio (e la schermata di salute e sicurezza), seleziona `Disable DSi menu H&S` e premi <kbd class="face">A</kbd> per confermare
...
Attualmente, Unlaunch apre il suo menu all'avvio per impostazione predefinita, ma può essere cambiarlo per avviare il menu DSi originale.
...
2. Entra in `OPZIONI`, e seleziona l'opzione `NO BUTTON`
3. Nell'elenco che segue, seleziona `Launcher`
4. Seleziona `SAVE & EXIT` per salvare le impostazioni, e spegni il tuo DSi
