...
2. `OPTIONS`に移動し、利用可能なオプションを見てください
   ...
   - The `NO BUTTON` (Which autoboots) and `BUTTON A / B / X / Y` options can be set however you like and will choose what your DSi loads at boot depending on which buttons are held. DSiWare、Homebrew (インストールしたメニューを含む)、Slot-1カード、wifiboot、UnlaunchのFilemenuのいずれかを選択できます
